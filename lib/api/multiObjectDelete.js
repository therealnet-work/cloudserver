...
function getObjMetadataAndDelete(authInfo, canonicalID, request,
    ...
    return async.forEachLimit(inPlay, 5, (entry, moveOn) => {
        async.waterfall([
            ...
            const reqObjOwnerMatch = canonicalID === objMD['owner-id'];
    ...
    // end of forEach func
    err => {
        const totalObjectsRemoved = deletedObjStats.requesterIsObjOwner.numOfObjectsRemoved +
            deletedObjStats.requesterNotObjOwner.numberOfObjectsRemoved;
    ...
    return async.waterfall([
        ...
        if (deletedObjStats.requesterNotObjOwner.numOfObjectsRemoved > 0) {
            pushMetric('multiObjectDelete', log, {
                ...
                bucket: bucketName,
                keys: deletedKeys,
                byteLength: Number.parseInt(deletedObjStats.requesterNotObjOwner.
                    totalContentLengthDeleted, 10),
